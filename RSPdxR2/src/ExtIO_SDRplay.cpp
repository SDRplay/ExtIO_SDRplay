...
/* *******************************************************************************/
/*					EXTIO Plugin Interface for SDRplay RSPdxR2					 */
/*									V1.0 build 0512								 */
/* Version 1.0 built against API 3.15                                            */
/*********************************************************************************/
...
#define INITIAL_GR      (41)

#define LNA_GAIN_SLIDER_MIN (0)
#define LNA_GAIN_SLIDER_MAX_0000_0012 (18)
#define LNA_GAIN_SLIDER_MAX_0012_0060 (19)
#define LNA_GAIN_SLIDER_MAX_0060_0250 (26)
#define LNA_GAIN_SLIDER_MAX_0250_0420 (27)
#define LNA_GAIN_SLIDER_MAX_0420_1000 (20)
#define LNA_GAIN_SLIDER_MAX_1000_2000 (18)
...
bool copyCSVtoHDD(char *filename)
{
	...
	string request = "GET /dx/sked-b18.csv HTTP/1.1\r\nHost: www.eibispace.de\r\n\r\n";
...
bool  LIBSDRplay_API __stdcall InitHW(char *name, char *model, int &type)
{
	...
	if (devices != NULL)
	{
		...
		for (i = 0; i < numDevs; i++)
		{
			...
			if (devices[i].hwVer == SDRPLAY_RSPdxR2_ID)
			{
				chosenDevice = i;
		...
		if (!foundRSPdxR2) {
			OutputDebugString("No RSPdxR2s found.");
			MessageBox(NULL, TEXT("Failed to find any available RSPdxR2s"), TEXT("SDRplay ExtIO Error"), MB_ICONERROR | MB_OK);
#ifdef DEBUG_ENABLE
			OutputDebugString("UnlockDeviceApi");
#endif
			sdrplay_api_UnlockDeviceApi_fn();
	...
	error = RegOpenKeyEx(HKEY_CURRENT_USER, TEXT("Software\\SDRplay\\RSPdxR2_Settings"), 0, KEY_ALL_ACCESS, &Settingskey);
	if (error == ERROR_SUCCESS)
	{
		...
		if (stationLookup)
		{
			if (SUCCEEDED(SHGetFolderPathW(NULL, CSIDL_APPDATA | CSIDL_FLAG_CREATE, NULL, 0, csvPath)))
			{
				...
				PathAppendW(csvPath, L"\\sked-b18.csv");
...
long LIBSDRplay_API __stdcall SetHWLO(unsigned long freq)
{
	...
	// if greater than 200 MHz and port C in use, then revert to 50 ohm port
	if (freq >= 200000000 && (deviceParams->devParams->rspDxParams.antennaSel == sdrplay_api_RspDx_ANTENNA_C))
	{
		// LNA Gain Reduction change limit
		AntennaIdx = 0;
		ComboBox_SetCurSel(GetDlgItem(h_dialog, IDC_ANTSELECT), AntennaIdx);
		SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0000_0012);
		deviceParams->devParams->rspDxParams.antennaSel = sdrplay_api_RspDx_ANTENNA_A;
		if (Running)
		{
			err = sdrplay_api_Update_fn(chosenDev->dev, chosenDev->tuner, sdrplay_api_Update_None, sdrplay_api_Update_RspDx_AntennaControl);
			if (err != sdrplay_api_Success)
			{
	...
	if (freq >= 1000000000) {
		if (LNAGainReduction > LNA_GAIN_SLIDER_MAX_1000_2000)
		{
			LNAGainReduction = LNA_GAIN_SLIDER_MAX_1000_2000;
		...
		SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_1000_2000);
	}
	else if (freq >= 420000000)
	{
		if (LNAGainReduction > LNA_GAIN_SLIDER_MAX_0420_1000)
		{
			LNAGainReduction = LNA_GAIN_SLIDER_MAX_0420_1000;
			sprintf_s(LNAGainReductionTxt, 254, "%d", LNAGainReduction);
			SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETPOS, (WPARAM)true, (LPARAM)LNAGainReduction);
			Edit_SetText(GetDlgItem(h_dialog, IDC_LNAGRNO), LNAGainReductionTxt);
		}

		SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0420_1000);
	}
	else if (freq >= 250000000)
	{
		if (LNAGainReduction > LNA_GAIN_SLIDER_MAX_0250_0420)
		{
			LNAGainReduction = LNA_GAIN_SLIDER_MAX_0250_0420;
			sprintf_s(LNAGainReductionTxt, 254, "%d", LNAGainReduction);
			SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETPOS, (WPARAM)true, (LPARAM)LNAGainReduction);
			Edit_SetText(GetDlgItem(h_dialog, IDC_LNAGRNO), LNAGainReductionTxt);
		}

		SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0250_0420);
	}
	else if (freq >= 60000000)
	{
		if (LNAGainReduction > LNA_GAIN_SLIDER_MAX_0060_0250)
		{
			LNAGainReduction = LNA_GAIN_SLIDER_MAX_0060_0250;
			sprintf_s(LNAGainReductionTxt, 254, "%d", LNAGainReduction);
			SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETPOS, (WPARAM)true, (LPARAM)LNAGainReduction);
			Edit_SetText(GetDlgItem(h_dialog, IDC_LNAGRNO), LNAGainReductionTxt);
		}

		SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0060_0250);
	}
	else if (freq >= 12000000)
	{
		if (LNAGainReduction > LNA_GAIN_SLIDER_MAX_0012_0060)
		{
			LNAGainReduction = LNA_GAIN_SLIDER_MAX_0012_0060;
			sprintf_s(LNAGainReductionTxt, 254, "%d", LNAGainReduction);
			SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETPOS, (WPARAM)true, (LPARAM)LNAGainReduction);
			Edit_SetText(GetDlgItem(h_dialog, IDC_LNAGRNO), LNAGainReductionTxt);
		}

		SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0012_0060);
	}
	else // 0 - 12 MHz
	{
		if (LNAGainReduction > LNA_GAIN_SLIDER_MAX_0000_0012)
		{
			LNAGainReduction = LNA_GAIN_SLIDER_MAX_0000_0012;
			sprintf_s(LNAGainReductionTxt, 254, "%d", LNAGainReduction);
			SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETPOS, (WPARAM)true, (LPARAM)LNAGainReduction);
			Edit_SetText(GetDlgItem(h_dialog, IDC_LNAGRNO), LNAGainReductionTxt);
		}
...
char * dayOfTheWeek()
{
	...
	sprintf_s(ret, 4, "%s", DAY[wday].c_str());
...
char * parseCSV(long freq)
{
	...
	if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_APPDATA | CSIDL_FLAG_CREATE, NULL, 0, csvPath)))
	{
		...
		PathAppend(csvPath, TEXT("\\sked-b18.csv"));
	}
	else
	{
...
{
	HKEY Settingskey;
	int error, AGC, LOAuto, PostDcComp, SL, tempVar;
	DWORD dwDisposition;
	...
	{ 
		if (error == ERROR_FILE_NOT_FOUND)
		{
			...
			error = RegCreateKeyEx(HKEY_CURRENT_USER, TEXT("Software\\SDRplay\\RSPdxR2_Settings"), 0, NULL, 0, KEY_ALL_ACCESS, NULL, &Settingskey, &dwDisposition);
		}
		else
		{
	...
	error = RegSetValueEx(Settingskey, TEXT("RefreshPeriodMemory"), 0, REG_DWORD, (LPBYTE)&RefreshPeriodMemory, sizeof(DcCompensationMode));
	if (error != ERROR_SUCCESS)
	...
	error = RegSetValueEx(Settingskey, TEXT("LNAGainReduction"), 0, REG_DWORD, (LPBYTE)&LNAGainReduction, sizeof(LNAGainReduction));
	if (error != ERROR_SUCCESS)
		MessageBox(NULL, "Failed to save LNA gain reduction value", NULL, MB_OK);

	error = RegSetValueEx(Settingskey, TEXT("AntennaIdx"), 0, REG_DWORD, (LPBYTE)&AntennaIdx, sizeof(AntennaIdx));
	if (error != ERROR_SUCCESS)
		MessageBox(NULL, "Failed to save Tuner setting", NULL, MB_OK);

	error = RegSetValueEx(Settingskey, TEXT("NotchEnable"), 0, REG_DWORD, (LPBYTE)&notchEnable, sizeof(notchEnable));
	if (error != ERROR_SUCCESS)
		MessageBox(NULL, "Failed to save Broadcast Notch setting", NULL, MB_OK);

	error = RegSetValueEx(Settingskey, TEXT("DABNotchEnable"), 0, REG_DWORD, (LPBYTE)&dabNotchEnable, sizeof(dabNotchEnable));
	if (error != ERROR_SUCCESS)
		MessageBox(NULL, "Failed to save DAB Notch setting", NULL, MB_OK);

	error = RegSetValueEx(Settingskey, TEXT("BiasTEnable"), 0, REG_DWORD, (LPBYTE)&biasTEnable, sizeof(biasTEnable));
	if (error != ERROR_SUCCESS)
		MessageBox(NULL, "Failed to save Bias T setting", NULL, MB_OK);
...
void LoadSettings()
{
	...
	if (error == ERROR_SUCCESS)
	{
		...
		error = RegQueryValueEx(Settingskey, "AntennaIdx", NULL, NULL, (LPBYTE)&AntennaIdx, &IntSz);
		if (error != ERROR_SUCCESS)
		{
#ifdef DEBUG_ENABLE
			OutputDebugString("Failed to recall saved Antenna Port");
#endif
			AntennaIdx = 0;
		}
		...
		case WM_NOTIFY:
			...
			case UDN_DELTAPOS:
			{
				if (((LPNMHDR)lParam)->idFrom == IDC_PPM_S)
				{
					...
					sprintf_s(Offset, 254, "%.2f", FqOffsetPPM);
...
//				TCHAR str1[255] = "dB";
		...
		case WM_INITDIALOG:
			...
			if (Frequency >= 1000.0)
				SendMessage(GetDlgItem(hwndDlg, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_1000_2000);
			else if (Frequency >= 420.0)
				SendMessage(GetDlgItem(hwndDlg, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0420_1000);
			else if (Frequency >= 250.0)
				SendMessage(GetDlgItem(hwndDlg, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0250_0420);
			else if (Frequency >= 60.0)
				SendMessage(GetDlgItem(hwndDlg, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0060_0250);
			else if (Frequency >= 12.0)
				SendMessage(GetDlgItem(hwndDlg, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0012_0060);
			else
				SendMessage(GetDlgItem(hwndDlg, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0000_0012);
			SendMessage(GetDlgItem(hwndDlg, IDC_LNASLIDER), TBM_SETPOS, (WPARAM)true, (LPARAM)LNAGainReduction);
			Edit_SetText(GetDlgItem(hwndDlg, IDC_LNAGRNO), LNAGainReductionTxt);

			//Configure for Antenna
			ComboBox_ResetContent(GetDlgItem(hwndDlg, IDC_ANTSELECT));
			_stprintf_s(str, 255, "Port A");
			ComboBox_AddString(GetDlgItem(hwndDlg, IDC_ANTSELECT), str);
			_stprintf_s(str, 255, "Port B");
			ComboBox_AddString(GetDlgItem(hwndDlg, IDC_ANTSELECT), str);
			_stprintf_s(str, 255, "Port C");
			ComboBox_AddString(GetDlgItem(hwndDlg, IDC_ANTSELECT), str);
			...
			// DAB Notch Enable
			SendMessage(GetDlgItem(hwndDlg, IDC_DABNOTCHENABLE), BM_SETCHECK, (WPARAM)(dabNotchEnable ? BST_CHECKED : BST_UNCHECKED), (LPARAM)0);
...
//				ShowWindow(h_ProfilesDialog, SW_SHOW);
		...
		case WM_CTLCOLORSTATIC:
			...
			case (IDC_TOTALGR):
			case (IDC_FINALSR):
			case (IDC_BIASTENABLE):
			case (IDC_PPMTXT):
			case (IDC_NOTCHENABLE):
			...
			case IDC_BIASTENABLE:
				if (ghwndDlg)	// update only when already initialized!
				{
					if (GET_WM_COMMAND_CMD(wParam, lParam) == BST_UNCHECKED)
					{
						...
						{
							biasTEnable = 1;
							deviceParams->devParams->rspDxParams.biasTEnable = (unsigned char)biasTEnable;
							if (Running)
								sdrplay_api_Update_fn(chosenDev->dev, chosenDev->tuner, sdrplay_api_Update_None, sdrplay_api_Update_RspDx_BiasTControl);
						}
						else
						{
							biasTEnable = 0;
			...
			case IDC_NOTCHENABLE:
				if (ghwndDlg)	// update only when already initialized!
				{
					if (GET_WM_COMMAND_CMD(wParam, lParam) == BST_UNCHECKED)
					{
						...
						{
							notchEnable = 1;
							deviceParams->devParams->rspDxParams.rfNotchEnable = (unsigned char)notchEnable;
							if (Running)
								sdrplay_api_Update_fn(chosenDev->dev, chosenDev->tuner, sdrplay_api_Update_None, sdrplay_api_Update_RspDx_RfNotchControl);
						}
						else
						{
							notchEnable = 0;
			...
			case IDC_DABNOTCHENABLE:
				if (ghwndDlg)	// update only when already initialized!
				{
					if (GET_WM_COMMAND_CMD(wParam, lParam) == BST_UNCHECKED)
					{
						...
						{
							dabNotchEnable = 1;
							deviceParams->devParams->rspDxParams.rfDabNotchEnable = (unsigned char)dabNotchEnable;
							if (Running)
								sdrplay_api_Update_fn(chosenDev->dev, chosenDev->tuner, sdrplay_api_Update_None, sdrplay_api_Update_RspDx_RfDabNotchControl);
						}
						else
						{
							dabNotchEnable = 0;
...
//							KillTimer(hwndDlg, ID_AGCUPDATE);
			...
			case IDC_ANTSELECT:
				if (GET_WM_COMMAND_CMD(wParam, lParam) == CBN_SELCHANGE)
				{
					...
					OutputDebugString(str);
#endif
					if (AntennaIdx == 0 && deviceParams->devParams->rspDxParams.antennaSel != sdrplay_api_RspDx_ANTENNA_A) // Port A
					{
						deviceParams->devParams->rspDxParams.antennaSel = sdrplay_api_RspDx_ANTENNA_A;
						if (Running)
						{
							sdrplay_api_Update_fn(chosenDev->dev, chosenDev->tuner, sdrplay_api_Update_None, sdrplay_api_Update_RspDx_AntennaControl);
						}
					}
					else if (AntennaIdx == 1 && deviceParams->devParams->rspDxParams.antennaSel != sdrplay_api_RspDx_ANTENNA_B) // Port B
					{
					...
					else if (AntennaIdx == 2 && deviceParams->devParams->rspDxParams.antennaSel != sdrplay_api_RspDx_ANTENNA_C) // Port B
					{
						if (Frequency < 200.0)
						{
			...
			if (GET_WM_COMMAND_CMD(wParam, lParam) == BN_CLICKED)
			{
				...
				if (GetOpenFileName(&ofn))
				{
					...
					else
					{
						...
						// AGC Enable
						ReadFile(hFile, &AGCEnabled, sizeof(AGCEnabled), &nBytesWritten, NULL);
						if (AGCEnabled)
						{
...
//							m_timer = SetTimer(h_dialog, ID_AGCUPDATE, 600, NULL);
						}
						else
						{
...
//							KillTimer(h_dialog, ID_AGCUPDATE);
						}
		...
		case IDC_SaveProfile:
			if (GET_WM_COMMAND_CMD(wParam, lParam) == BN_CLICKED)
			{
				TCHAR writevers[5] = TEXT("0001");
				int _BandwidthIdx = BandwidthIdx;
				int _GainReduction = GainReduction;
				int _LNAGainReduction = LNAGainReduction;
				ofn.lStructSize = sizeof(OPENFILENAME);
				ofn.hwndOwner = h_ProfilesDialog;
				ofn.lpstrFilter = _T("RSP Profiles (*.rsp)\0*.rsp\0\0");
				ofn.lpstrFile = szFileName;
				...
				if (GetSaveFileName(&ofn))
				{
					...
					WriteFile(hFile, &AGCEnabled, sizeof(AGCEnabled), &nBytesWritten, NULL);							// AGC Enable
					...
					WriteFile(hFile, &_LNAGainReduction, sizeof(_LNAGainReduction), &nBytesWritten, NULL);				// LNA gain reduction value
					WriteFile(hFile, &AntennaIdx, sizeof(AntennaIdx), &nBytesWritten, NULL);							// Port
					WriteFile(hFile, &notchEnable, sizeof(notchEnable), &nBytesWritten, NULL);							// Broadcast Notch filter enable
					WriteFile(hFile, &dabNotchEnable, sizeof(dabNotchEnable), &nBytesWritten, NULL);					// DAB Notch filter enable
					WriteFile(hFile, &biasTEnable, sizeof(biasTEnable), &nBytesWritten, NULL);							// Bias T enable
...
//			KillTimer(h_dialog, ID_AGCUPDATE);
		...
		SendMessage(GetDlgItem(h_dialog, IDC_NOTCHENABLE), BM_SETCHECK, (WPARAM)(notchEnable ? BST_CHECKED : BST_UNCHECKED), (LPARAM)0);

		// DAB Notch Enable
		ReadFile(hFile, &dabNotchEnable, sizeof(dabNotchEnable), &nBytesWritten, NULL);
		SendMessage(GetDlgItem(h_dialog, IDC_DABNOTCHENABLE), BM_SETCHECK, (WPARAM)(dabNotchEnable ? BST_CHECKED : BST_UNCHECKED), (LPARAM)0);

		// Bias T Enable
		ReadFile(hFile, &biasTEnable, sizeof(biasTEnable), &nBytesWritten, NULL);
		SendMessage(GetDlgItem(h_dialog, IDC_BIASTENABLE), BM_SETCHECK, (WPARAM)(biasTEnable ? BST_CHECKED : BST_UNCHECKED), (LPARAM)0);
