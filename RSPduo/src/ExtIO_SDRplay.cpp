...
#define LNA_GAIN_SLIDER_MAX (9)
#define LNA_GAIN_SLIDER_MAX_AM_50OHM (6)
#define LNA_GAIN_SLIDER_MAX_AM_HIZ (4)
#define LNA_GAIN_SLIDER_MAX_LBAND  (8)
		...
		for (i = 0; i < numDevs; i++)
		{
			...
			if (devices[i].hwVer == SDRPLAY_RSPduo_ID)
			{
				...
				{	// Single Tuner
					...
					chosenDev->rspDuoSampleFreq = 2.0 * 1e6;
				}
				else
				{	// Slave
...
long LIBSDRplay_API __stdcall SetHWLO(unsigned long freq)
{
	...
	if (freq >= 30000000 && (chosenDev->tuner & sdrplay_api_Tuner_A) && (chParams->rspDuoTunerParams.tuner1AmPortSel == sdrplay_api_RspDuo_AMPORT_1))
	{
		...
		if (freq >= 1000000000) {
			if (LNAGainReduction > LNA_GAIN_SLIDER_MAX_LBAND)
			{
				LNAGainReduction = LNA_GAIN_SLIDER_MAX_LBAND;
		...
		{
			if (LNAGainReduction > LNA_GAIN_SLIDER_MAX_AM_50OHM)
			{
				LNAGainReduction = LNA_GAIN_SLIDER_MAX_AM_50OHM;
				sprintf_s(LNAGainReductionTxt, 254, "%d", LNAGainReduction);
				SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETPOS, (WPARAM)true, (LPARAM)LNAGainReduction);
				Edit_SetText(GetDlgItem(h_dialog, IDC_LNAGRNO), LNAGainReductionTxt);
			}
...
void SaveSettings()
{
	...
	{ 
		if (error == ERROR_FILE_NOT_FOUND)
		{
			...
			error = RegCreateKeyEx(HKEY_CURRENT_USER, TEXT("Software\\SDRplay\\RSPduo_Settings"), 0, NULL, 0, KEY_ALL_ACCESS, NULL, &Settingskey, &dwDisposition);
		}
		else
		{
	...
	error = RegSetValueEx(Settingskey, TEXT("RefClkEnable"), 0, REG_DWORD, (LPBYTE)&refClkEnable, sizeof(refClkEnable));
	if (error != ERROR_SUCCESS)
		MessageBox(NULL, "Failed to save Reference Clk setting", NULL, MB_OK);

	error = RegSetValueEx(Settingskey, TEXT("TunerIdx"), 0, REG_DWORD, (LPBYTE)&TunerIdx, sizeof(TunerIdx));
	if (error != ERROR_SUCCESS)
		MessageBox(NULL, "Failed to save Tuner setting", NULL, MB_OK);

	error = RegSetValueEx(Settingskey, TEXT("HiZPort"), 0, REG_DWORD, (LPBYTE)&HiZPort, sizeof(HiZPort));
	if (error != ERROR_SUCCESS)
		MessageBox(NULL, "Failed to save HiZ Port setting", NULL, MB_OK);
...
//				TCHAR str1[255] = "dB";
		...
		case WM_INITDIALOG:
			...
			if (HiZPort == 1)
				SendMessage(GetDlgItem(hwndDlg, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_AM_HIZ);
			else
				if (Frequency >= 1000.0)
					SendMessage(GetDlgItem(hwndDlg, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_LBAND);
				else if (Frequency < 30.0)
					SendMessage(GetDlgItem(hwndDlg, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_AM_50OHM);
				else
			...
			// Ref Clock Enable
			SendMessage(GetDlgItem(hwndDlg, IDC_REFCLOCKOUT), BM_SETCHECK, (WPARAM)(refClkEnable ? BST_CHECKED : BST_UNCHECKED), (LPARAM)0);

			//Configure for Tuner 1 Default
			ComboBox_ResetContent(GetDlgItem(hwndDlg, IDC_TUNERSELECT));
			_stprintf_s(str, 255, "Tuner 1");
			ComboBox_AddString(GetDlgItem(hwndDlg, IDC_TUNERSELECT), str);
			_stprintf_s(str, 255, "Tuner 2");
			ComboBox_AddString(GetDlgItem(hwndDlg, IDC_TUNERSELECT), str);
			ComboBox_SetCurSel(GetDlgItem(hwndDlg, IDC_TUNERSELECT), TunerIdx);

			// Clear HiZ Port
			Edit_Enable(GetDlgItem(hwndDlg, IDC_HIZPORT), 1);
			SendMessage(GetDlgItem(h_dialog, IDC_HIZPORT), BM_SETCHECK, (WPARAM)(0), 0);

			if (chosenDev->rspDuoMode == sdrplay_api_RspDuoMode_Slave)
				Edit_Enable(GetDlgItem(hwndDlg, IDC_TUNERSELECT), 0);

			//Setup Mode Dropdown
			if (chosenDev->rspDuoMode == sdrplay_api_RspDuoMode_Slave)
			{
				ComboBox_ResetContent(GetDlgItem(hwndDlg, IDC_MODESELECT));
				_stprintf_s(str, 255, "Slave");
				ComboBox_AddString(GetDlgItem(hwndDlg, IDC_MODESELECT), str);
				ComboBox_SetCurSel(GetDlgItem(hwndDlg, IDC_MODESELECT), 0);
				Edit_Enable(GetDlgItem(hwndDlg, IDC_MODESELECT), 0);
			}
			else
			{
				ComboBox_ResetContent(GetDlgItem(hwndDlg, IDC_MODESELECT));
				_stprintf_s(str, 255, "Single");
				ComboBox_AddString(GetDlgItem(hwndDlg, IDC_MODESELECT), str);
				_stprintf_s(str, 255, "(Dual) Normal");
				ComboBox_AddString(GetDlgItem(hwndDlg, IDC_MODESELECT), str);
				_stprintf_s(str, 255, "(Dual) ADS-B");
				ComboBox_AddString(GetDlgItem(hwndDlg, IDC_MODESELECT), str);
				ComboBox_SetCurSel(GetDlgItem(hwndDlg, IDC_MODESELECT), ModeIdx);
			}
...
//							KillTimer(hwndDlg, ID_AGCUPDATE);
			...
			case IDC_TUNERSELECT:
				if (GET_WM_COMMAND_CMD(wParam, lParam) == CBN_SELCHANGE)
				{
					TunerIdx = ComboBox_GetCurSel(GET_WM_COMMAND_HWND(wParam, lParam));
#ifdef DEBUG_ENABLE
					...
					if (GET_WM_COMMAND_CMD(wParam, lParam) == BST_UNCHECKED)
					{
						if (Button_GetCheck(GET_WM_COMMAND_HWND(wParam, lParam)) == BST_CHECKED) //it is checked
						{
							if (Frequency < 30.0)
							{
								...
								{
									LNAGainReduction = LNA_GAIN_SLIDER_MAX_AM_HIZ;
									sprintf_s(LNAGainReductionTxt, 254, "%d", LNAGainReduction);
									SendMessage(GetDlgItem(hwndDlg, IDC_LNASLIDER), TBM_SETPOS, (WPARAM)true, (LPARAM)LNAGainReduction);
									Edit_SetText(GetDlgItem(hwndDlg, IDC_LNAGRNO), LNAGainReductionTxt);
								}
			...
			case IDC_MODESELECT:
				if (GET_WM_COMMAND_CMD(wParam, lParam) == CBN_SELCHANGE)
				{
					int oldModeIdx = ModeIdx;
					ModeIdx = ComboBox_GetCurSel(GET_WM_COMMAND_HWND(wParam, lParam));
#ifdef DEBUG_ENABLE
					sprintf_s(str, 255, "ModeIdx: %d", ModeIdx);
					OutputDebugString(str);
#endif
					if (ModeIdx == 0)	// Single
					{
						...
						{
#ifdef DEBUG_ENABLE
							OutputDebugString("Switch to Single mode");
#endif
							err = sdrplay_api_ReleaseDevice_fn(chosenDev);
							if (err != sdrplay_api_Success)
							{
								_stprintf_s(str, 255, "ReleaseDevice Error: %s", sdrplay_api_GetErrorString_fn(err));
								OutputDebugString(str);
							}
							...
							if (err != sdrplay_api_Success)
							{
								_stprintf_s(str, 255, "SelectDevice Error: %s", sdrplay_api_GetErrorString_fn(err));
								OutputDebugString(str);
							}
							...
							if (err != sdrplay_api_Success)
							{
								_stprintf_s(str, 255, "GetDeviceParams Error: %s", sdrplay_api_GetErrorString_fn(err));
								OutputDebugString(str);
							}
							...
							Edit_Enable(GetDlgItem(hwndDlg, IDC_TUNERSELECT), 1);

							if (chosenDev->tuner == sdrplay_api_Tuner_A || chosenDev->tuner == sdrplay_api_Tuner_Both)
							{
								Edit_Enable(GetDlgItem(hwndDlg, IDC_HIZPORT), 1);
								if ((Button_GetCheck(GetDlgItem(hwndDlg, IDC_HIZPORT)) == BST_CHECKED) && Frequency < 30.0)
								{
									chParams->rspDuoTunerParams.tuner1AmPortSel = sdrplay_api_RspDuo_AMPORT_1;
									HiZPort = 1;
									Edit_Enable(GetDlgItem(hwndDlg, IDC_HIZPORT), 1);
					...
					else if (ModeIdx == 1)				// Master/Slave Normal
					{
#ifdef DEBUG_ENABLE
						OutputDebugString("LowIF Normal");
#endif
						err = sdrplay_api_ReleaseDevice_fn(chosenDev);
						if (err != sdrplay_api_Success)
						{
					...
					else if (ModeIdx == 2)			// Master/Slave ADS-B
					{
#ifdef DEBUG_ENABLE
						OutputDebugString("LowIF ADS-B");
#endif
						err = sdrplay_api_ReleaseDevice_fn(chosenDev);
						if (err != sdrplay_api_Success)
						{
...
//					WinradCallBack(-1, WINRAD_SRCHANGE, 0, NULL);
					...
					// HiZ Port Not Selected
					if (chosenDev->tuner == sdrplay_api_Tuner_A || chosenDev->tuner == sdrplay_api_Tuner_Both)
					{
						TunerIdx = 0;
						HiZPort = 0;
						Edit_Enable(GetDlgItem(hwndDlg, IDC_HIZPORT), 1);
						SendMessage(GetDlgItem(hwndDlg, IDC_HIZPORT), BM_SETCHECK, (WPARAM)(0), HiZPort);
					}
					else
					{
						TunerIdx = 1;
		...
		case IDC_SaveProfile:
			if (GET_WM_COMMAND_CMD(wParam, lParam) == BN_CLICKED)
			{
				...
				if (GetSaveFileName(&ofn))
				{
					...
					WriteFile(hFile, &refClkEnable, sizeof(refClkEnable), &nBytesWritten, NULL);						// Reference Clock Enable
					WriteFile(hFile, &TunerIdx, sizeof(TunerIdx), &nBytesWritten, NULL);								// Tuner/Port
...
//			KillTimer(h_dialog, ID_AGCUPDATE);
		...
		// Ref Clock Enable
		ReadFile(hFile, &refClkEnable, sizeof(refClkEnable), &nBytesWritten, NULL);
		SendMessage(GetDlgItem(h_dialog, IDC_REFCLOCKOUT), BM_SETCHECK, (WPARAM)(refClkEnable ? BST_CHECKED : BST_UNCHECKED), (LPARAM)0);
		...
		SendMessage(GetDlgItem(h_dialog, IDC_HIZPORT), BM_SETCHECK, (WPARAM)(HiZPort ? BST_CHECKED : BST_UNCHECKED), (LPARAM)0);
...
void Reset_SAMPLERATE(HWND hwndDlg)
{
	...
	if ((chosenDev->rspDuoMode & sdrplay_api_RspDuoMode_Master && !slaveAttached) || chosenDev->rspDuoMode & sdrplay_api_RspDuoMode_Single_Tuner)
	{
		if (IFmodeIdx == 0)
		{
			chosenDev->rspDuoSampleFreq = samplerates[SampleRateIdx].value * 1e6;
			deviceParams->devParams->fsFreq.fsHz = samplerates[SampleRateIdx].value * 1e6;
		}
		else
		{
