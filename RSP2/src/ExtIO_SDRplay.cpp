...
#define LNA_GAIN_SLIDER_MAX  (8)
#define LNA_GAIN_SLIDER_MAX_AMPORT2  (4)
#define LNA_GAIN_SLIDER_MAX_BAND45  (5)
...
long LIBSDRplay_API __stdcall SetHWLO(unsigned long freq)
{
	...
	if (freq >= 30000000 && AntennaIdx == 2) // i.e. greater than 30MHz and HiZ port in use, then revert to port A
	{
	...
	if (freq >= 420000000)
	{
		...
		SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_BAND45);
...
void SaveSettings()
{
	...
	{ 
		if (error == ERROR_FILE_NOT_FOUND)
		{
			...
			error = RegCreateKeyEx(HKEY_CURRENT_USER, TEXT("Software\\SDRplay\\RSP2_Settings"), 0, NULL, 0, KEY_ALL_ACCESS, NULL, &Settingskey, &dwDisposition);
		}
		else
		{
	...
	if (error != ERROR_SUCCESS)
		MessageBox(NULL, "Failed to save Antenna setting", NULL, MB_OK);
		...
		case IDC_SaveProfile:
			if (GET_WM_COMMAND_CMD(wParam, lParam) == BN_CLICKED)
			{
				...
				if (GetSaveFileName(&ofn))
				{
					...
					WriteFile(hFile, &AntennaIdx, sizeof(AntennaIdx), &nBytesWritten, NULL);							// Antenna port
...
//			KillTimer(h_dialog, ID_AGCUPDATE);
		...
		sprintf_s(str, sizeof(str), "%d", refClkEnable);
