...
bool  LIBSDRplay_API __stdcall InitHW(char *name, char *model, int &type)
{
	...
	if (devices != NULL)
	{
		...
		if (!foundRSPdx) {
			OutputDebugString("No RSPdxs found.");
			MessageBox(NULL, TEXT("Failed to find any available RSPdxs"), TEXT("SDRplay ExtIO Error"), MB_ICONERROR | MB_OK);
#ifdef DEBUG_ENABLE
			OutputDebugString("UnlockDeviceApi");
#endif
			sdrplay_api_UnlockDeviceApi_fn();
	...
	error = RegOpenKeyEx(HKEY_CURRENT_USER, TEXT("Software\\SDRplay\\RSPdx_Settings"), 0, KEY_ALL_ACCESS, &Settingskey);
...
void SaveSettings()
{
	...
	{ 
		if (error == ERROR_FILE_NOT_FOUND)
		{
			...
			error = RegCreateKeyEx(HKEY_CURRENT_USER, TEXT("Software\\SDRplay\\RSPdx_Settings"), 0, NULL, 0, KEY_ALL_ACCESS, NULL, &Settingskey, &dwDisposition);
		}
		else
		{
