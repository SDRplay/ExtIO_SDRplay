...
/* *******************************************************************************/
/*					EXTIO Plugin Interface for SDRplay RSP1B					 */
/*									V1.0 build 0122								 */
/* Version 1.0 built against API 3.11											 */			
/*********************************************************************************/
...
#define LNA_GAIN_SLIDER_MAX_0000_0050 (6)
#define LNA_GAIN_SLIDER_MAX_0050_1000 (9)
#define LNA_GAIN_SLIDER_MAX_1000_2000 (8)
...
bool  LIBSDRplay_API __stdcall InitHW(char *name, char *model, int &type)
{
	...
	if (devices != NULL)
	{
		...
		for (i = 0; i < numDevs; i++)
		{
			...
			if (devices[i].hwVer == SDRPLAY_RSP1B_ID)
			{
				chosenDevice = i;
		...
		if (!foundRSP1B) {
			OutputDebugString("No RSP1Bs found.");
			MessageBox(NULL, TEXT("Failed to find any available RSP1Bs"), TEXT("SDRplay ExtIO Error"), MB_ICONERROR | MB_OK);
#ifdef DEBUG_ENABLE
			OutputDebugString("UnlockDeviceApi");
#endif
			sdrplay_api_UnlockDeviceApi_fn();
	...
	error = RegOpenKeyEx(HKEY_CURRENT_USER, TEXT("Software\\SDRplay\\RSP1B_Settings"), 0, KEY_ALL_ACCESS, &Settingskey);
...
long LIBSDRplay_API __stdcall SetHWLO(unsigned long freq)
{
	...
	{
		if (LNAGainReduction > LNA_GAIN_SLIDER_MAX_0050_1000)
		{
			LNAGainReduction = LNA_GAIN_SLIDER_MAX_0050_1000;
			sprintf_s(LNAGainReductionTxt, 254, "%d", LNAGainReduction);
			SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETPOS, (WPARAM)true, (LPARAM)LNAGainReduction);
			Edit_SetText(GetDlgItem(h_dialog, IDC_LNAGRNO), LNAGainReductionTxt);
		}

		SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0050_1000);
	}
	else // 0 - 50 MHz
	{
		if (LNAGainReduction > LNA_GAIN_SLIDER_MAX_0000_0050)
		{
			LNAGainReduction = LNA_GAIN_SLIDER_MAX_0000_0050;
			sprintf_s(LNAGainReductionTxt, 254, "%d", LNAGainReduction);
			SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETPOS, (WPARAM)true, (LPARAM)LNAGainReduction);
			Edit_SetText(GetDlgItem(h_dialog, IDC_LNAGRNO), LNAGainReductionTxt);
		}

		SendMessage(GetDlgItem(h_dialog, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0000_0050);
	}
...
void SaveSettings()
{
	...
	{ 
		if (error == ERROR_FILE_NOT_FOUND)
		{
			...
			error = RegCreateKeyEx(HKEY_CURRENT_USER, TEXT("Software\\SDRplay\\RSP1B_Settings"), 0, NULL, 0, KEY_ALL_ACCESS, NULL, &Settingskey, &dwDisposition);
		}
		else
		{
	...
	if (error != ERROR_SUCCESS)
		MessageBox(NULL, "Failed to save Notch setting", NULL, MB_OK);
...
void LoadSettings()
{
	...
	if (error == ERROR_SUCCESS)
	{
		...
		{
#ifdef DEBUG_ENABLE
			OutputDebugString("Failed to recall saved Notch value");
#endif
			notchEnable = 0;
		}
...
//				TCHAR str1[255] = "dB";
		...
		case WM_INITDIALOG:
			...
			if (Frequency >= 1000.0)
				...
				SendMessage(GetDlgItem(hwndDlg, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0050_1000);
			else
				SendMessage(GetDlgItem(hwndDlg, IDC_LNASLIDER), TBM_SETRANGEMAX, (WPARAM)true, (LPARAM)LNA_GAIN_SLIDER_MAX_0000_0050);
			...
			case IDC_BIASTENABLE:
				if (ghwndDlg)	// update only when already initialized!
				{
					if (GET_WM_COMMAND_CMD(wParam, lParam) == BST_UNCHECKED)
					{
						...
						{
							biasTEnable = 1;
							deviceParams->rxChannelA->rsp1aTunerParams.biasTEnable = (unsigned char)biasTEnable;
							if (Running)
								sdrplay_api_Update_fn(chosenDev->dev, chosenDev->tuner, sdrplay_api_Update_Rsp1a_BiasTControl, sdrplay_api_Update_Ext1_None);
						}
						else
						{
							biasTEnable = 0;
			...
			case IDC_NOTCHENABLE:
				if (ghwndDlg)	// update only when already initialized!
				{
					if (GET_WM_COMMAND_CMD(wParam, lParam) == BST_UNCHECKED)
					{
						...
						{
							notchEnable = 1;
							deviceParams->devParams->rsp1aParams.rfNotchEnable = (unsigned char)notchEnable;
							if (Running)
								sdrplay_api_Update_fn(chosenDev->dev, chosenDev->tuner, sdrplay_api_Update_Rsp1a_RfNotchControl, sdrplay_api_Update_Ext1_None);
						}
						else
						{
							notchEnable = 0;
			...
			case IDC_DABNOTCHENABLE:
				if (ghwndDlg)	// update only when already initialized!
				{
					if (GET_WM_COMMAND_CMD(wParam, lParam) == BST_UNCHECKED)
					{
						...
						{
							dabNotchEnable = 1;
							deviceParams->devParams->rsp1aParams.rfDabNotchEnable = (unsigned char)dabNotchEnable;
							if (Running)
								sdrplay_api_Update_fn(chosenDev->dev, chosenDev->tuner, sdrplay_api_Update_Rsp1a_RfDabNotchControl, sdrplay_api_Update_Ext1_None);
						}
						else
						{
							dabNotchEnable = 0;
		...
		case IDC_SaveProfile:
			if (GET_WM_COMMAND_CMD(wParam, lParam) == BN_CLICKED)
			{
				...
				if (GetSaveFileName(&ofn))
				{
					...
					WriteFile(hFile, &notchEnable, sizeof(notchEnable), &nBytesWritten, NULL);							// Notch filter enable
...
//			KillTimer(h_dialog, ID_AGCUPDATE);
		...
		sprintf_s(str, sizeof(str), "%d", notchEnable);
		...
		sprintf_s(str, sizeof(str), "%d", dabNotchEnable);
		...
		sprintf_s(str, sizeof(str), "%d", biasTEnable);
